...
@router.post(
    ...
    description=(
        "Menjalankan beberapa langkah sekaligus untuk satu lokasi: crawling, "
        "analisis AI, dan export CSV — masing-masing bisa di-toggle (`fetch`, "
        "`analyze`, `export_csv`). "
...
def run_location_pipeline(
    payload: LocationPipelineRequest,
    ...
    if payload.fetch:
        if payload.dry_run:
            ...
            fetch_result = FetchService(
                company_id=principal.company_id
            ).fetch_location(payload.location_id)
    ...
    if payload.export_csv and not payload.dry_run:
        export_path = ExportService(
            company_id=principal.company_id
        ).export_location_reviews_csv(payload.location_id)
        result["steps"]["export"] = {
            "status": "success",
