    ...
    def analyze_review(self, review: dict) -> dict:
        response = self._http.post(
            ...
            json={
                ...
                "profile": self.settings.jev_profile,
                "confidence_threshold": self.settings.jev_confidence_threshold,
            },
        )
        if response.status_code >= 400:
            raise RuntimeError(f"Jev AI returned HTTP {response.status_code}.")
        payload = response.json()
        if not isinstance(payload, dict) or not isinstance(payload.get("results"), list):
            raise TypeError("Jev AI returned an invalid response.")
